...
namespace PicoContainer.Core.Alternatives
{
	...
	public class ImplementationHidingCachingPicoContainer : AbstractDelegatingMutablePicoContainer
	{
		...
		private ImplementationHidingCachingPicoContainer(IPicoContainer parent, CachingComponentAdapterFactory caf, ILifecycleManager lifecycleManager) 
		...
		public override IComponentAdapter RegisterComponentImplementation(Object componentKey, Type componentImplementation)
		{
			if (componentKey is Type)
			{
				...
				if (clazz.IsInterface)
				{
					...
					return DelegateContainer.RegisterComponent(new CachingComponentAdapter(new ImplementationHidingComponentAdapter(caDelegate, true)));
				}
			}
