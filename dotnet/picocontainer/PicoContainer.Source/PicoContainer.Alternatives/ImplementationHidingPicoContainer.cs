...
namespace PicoContainer.Alternatives
{
	...
	{
		private IComponentAdapterFactory caf;
		...
		public ImplementationHidingPicoContainer(IComponentAdapterFactory caf, IPicoContainer parent, ILifecycleManager lifecycleManager) : base(new DefaultPicoContainer(caf, parent, lifecycleManager))
		{
			this.caf = caf;
			this.lifecycleManager = lifecycleManager;
		}

		public ImplementationHidingPicoContainer(IComponentAdapterFactory caf, IPicoContainer parent) : this(caf, parent, new DefaultLifecycleManager())
		{
		}
		...
		/// </summary>
		/// <param name="parent"></param>
		public ImplementationHidingPicoContainer(IPicoContainer parent) : this(new DefaultComponentAdapterFactory(), parent)
		{
		}
		...
		public override IComponentAdapter RegisterComponentImplementation(Object componentKey, Type componentImplementation)
		{
			if (componentKey is Type)
			{
				...
				{
					IComponentAdapter caDelegate = caf.CreateComponentAdapter(componentKey, componentImplementation, null);
					return DelegateContainer.RegisterComponent(new ImplementationHidingComponentAdapter(caDelegate, true));
				}
			}
