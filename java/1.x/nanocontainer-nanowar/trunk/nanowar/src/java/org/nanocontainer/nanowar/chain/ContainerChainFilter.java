...
public class ContainerChainFilter implements Filter {
	...
	public void init(FilterConfig config) throws ServletException {
		...
		if (divertorKey != null) {
			retriever = new DivertorRetriever() {
				...
				public Divertor getDivertor(PicoContainer container) {
					return (Divertor) container
							.getComponentInstance(divertorKey);
				}
			...
			retriever = new DivertorRetriever() {
				public Divertor getDivertor(PicoContainer container) {
					return (Divertor) container
							.getComponentInstanceOfType(divertorClass);
				}
			};
		} else {
			retriever = new DivertorRetriever() {
				public Divertor getDivertor(PicoContainer container) {
					return (Divertor) container
							.getComponentInstanceOfType(Divertor.class);
				}
			};
		}
	}
	 ...
	 * 
	 * @param url
	 *            the String with the original url
