...
/**
 * ScopedContainerComposer is a ContainerComposer which can build a PicoContainer for different web context scopes:
 * application, session and request. The configuration for each scope is contained in one of more NanoContainer scripts.
 * The ContainerBuilder used to build the PicoContainer and the names of scoped script files are configurable via a
 * ScopedContainerConfigurator.
 * 
 * @author Mauro Talevi
...
public class ScopedContainerComposer implements ContainerComposer {
	...
	private ScopedContainerConfigurator getConfigurator(PicoContainer pico) {
		ScopedContainerConfigurator configurator = (ScopedContainerConfigurator) pico
				.getComponentInstance(ScopedContainerConfigurator.class);
