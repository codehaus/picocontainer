...
/**
 * Servlet listener class that hooks into the underlying servlet container and instantiates, assembles, starts, stores
 * and disposes the appropriate pico containers when applications/sessions start/stop. <p/> To use, add this as a
 * listener-class to web.xml. The containers are configured via context-params in web.xml, in two ways:
 * <ol>
 * <li>A NanoContainer script via a parameter whose name is nanocontainer.<language>, where <language> is one of the
 * supported scripting languages, see {@link ScriptedContainerBuilderFactory}. The parameter value can be either an
 * inlined script (enclosed in <![CDATA[]>), or a resource path for the script. </li>
 * <li>A ContainerComposer class via the parameter name {@link CONTAINER_COMPOSER}, which can be configured via an
 * optional parameter {@link CONTAINER_COMPOSER_CONFIGURATION}. </li>
 * </ol>
 * 
...
public class ServletContainerListener implements ServletContextListener, HttpSessionListener, KeyConstants, Serializable {
	...
	private ContainerBuilder createBuilder(ServletContext context) throws ClassNotFoundException {
		...
		while (initParameters.hasMoreElements()) {
			...
			if (initParameter.startsWith(NANOCONTAINER_PREFIX)) {
				...
				ScriptedContainerBuilderFactory scriptedContainerBuilderFactory = new ScriptedContainerBuilderFactory(scriptReader,
	...
	private ContainerComposer createContainerComposer(ServletContext context) throws ClassNotFoundException {
		...
		if (picoConfiguration != null) {
			...
			componentAdapter = nanoContainer.registerComponentImplementation(containerComposerClassName, containerComposerClassName,
					parameters);
		} else {
