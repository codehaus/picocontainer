...
/**
 * <p>
 * A Filter which delegates to another Filter which is registered in a PicoContainer, in any of the web scopes, context,
 * session or request.
 * </p>
 * <p>
 * The delegate Filter must be registered via the "delegate-key" or "delegate-class" init-params of this Filter.
 * <p>
 * The initialization is done lazily, using the <code>init-type</code> init-param to control it. Allowed values are:
 * <ul>
 ...
 * </p>
 * <p>
 * The lookup in the PicoContainer is by default done for each request, but you can control that behaviour with the
 * <code>lookup-only-once</code> init parameter. If set to "true", then PicoFilterProxy will only lookup your delegate
 * filter at the first request.
 * </p>
 * <p>
 * <b>Note</b>: Be aware that any dependency on your filter, in this setup, will stay referenced by your filter for its
 * whole lifetime, eventhough this dependency might have been set up at request level in your composer!
 * </p>
 * 
	...
	private Filter lookupDelegate(HttpServletRequest request) throws ServletException {
		...
		if (delegateClassName != null) {
			...
			throw new PicoInitializationException(
					"You must specify one of delegate-class or delegate-key in the filter config, and you must register the corresponding component in your PicoContainer");
		}
