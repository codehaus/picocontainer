...
/**
 * container composer reading from xml files via xstream. it pulls configuration from xml files placed on classpath of
 * web application ( WEB-INF/classes/ ) , and their names are: nano-application.xml , nano-session.xml and
 * nano-request.xml
 * 
...
public class XStreamContainerComposer implements ContainerComposer {
	...
	public XStreamContainerComposer() {
		...
		InputStreamReader requestScopeScript = new InputStreamReader(Thread.currentThread().getContextClassLoader().getResourceAsStream(
				REQUEST_CONFIG));
		XStreamContainerBuilder requestPopulator = new XStreamContainerBuilder(requestScopeScript, Thread.currentThread()
				.getContextClassLoader());
		requestPopulator.populateContainer(requestRecorder.getContainerProxy());

		// create and populate session scope
		InputStreamReader sessionScopeScript = new InputStreamReader(Thread.currentThread().getContextClassLoader().getResourceAsStream(
				SESSION_CONFIG));
		XStreamContainerBuilder sessionPopulator = new XStreamContainerBuilder(sessionScopeScript, Thread.currentThread()
				.getContextClassLoader());
	...
	public void composeContainer(MutablePicoContainer container, Object scope) {
		if (scope instanceof ServletContext) {
			InputStreamReader applicationScopeScript = new InputStreamReader(Thread.currentThread().getContextClassLoader()
					.getResourceAsStream(APPLICATION_CONFIG));
			XStreamContainerBuilder applicationPopulator = new XStreamContainerBuilder(applicationScopeScript, Thread.currentThread()
					.getContextClassLoader());
