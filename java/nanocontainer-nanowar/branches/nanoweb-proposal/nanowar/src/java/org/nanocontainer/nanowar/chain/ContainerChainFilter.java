 ...
 * </p>
 * <p>
 * Chain is wired to request container and substituted with request scoped reference so nobody notices additional
 * containers.
 * </p>
 * <p>
 ...
 * <li>emptyContainerScript: specifies the empty container script (as a String) to use if the path container script is
 * not found</li>
 * </ul>
 * </p>
 * <p>
...
public class ContainerChainFilter implements Filter {
	...
	public void init(FilterConfig config) throws ServletException {
		...
		String builderClassName = config.getInitParameter(BUILDER_CLASSNAME_PARAM);
		String containerScriptName = config.getInitParameter(CONTAINER_SCRIPT_NAME_PARAM);
		String emptyContainerScript = config.getInitParameter(EMPTY_CONTAINER_SCRIPT_PARAM);
		checkParametersAreSet(builderClassName, containerScriptName, emptyContainerScript);
		chainBuilder = new ServletChainBuilder(config.getServletContext(), builderClassName, containerScriptName, emptyContainerScript);
	}

	/**
	 * Instantiates ChainMonitor from class name or a ConsoleChainMonitor if class name not provided or invalid
	 * 
	...
	private ChainMonitor createMonitor(String monitorClassName) {
		...
		try {
			return (ChainMonitor) classLoader.loadClass(monitorClassName).newInstance();
		} catch (Exception e) {
	 ...
	 */
	private void checkParametersAreSet(String builderClassName, String containerScriptName, String emptyContainerScript)
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
		...
		if (httpRequest.getAttribute(ALREADY_FILTERED_KEY) == null) {
			...
			try {
				...
				// build chain
				ContainerChain chain = chainBuilder.buildChain(elements.toArray(), container);
				// start chain
				chain.start();
	...
	private ObjectReference obtainRequestObjectReference(ServletRequest request) {
		return new RequestScopeObjectReference(request, KeyConstants.REQUEST_CONTAINER);
	}
	...
	private void handleException(Exception e, PicoContainer container, ServletRequest request, ServletResponse response)
	...
	private List extractPathElements(String url) {
		...
		for (int pos = url.indexOf(PATH_SEPARATOR, 1); pos > 0; pos = url.indexOf(PATH_SEPARATOR, pos + 1)) {
