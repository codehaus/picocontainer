...
/**
 * velocity integration servlet. integrates container hieararchy into velocity context as well webwork specific objects.
 * This servlet is not derived from standart webwork velocity servlet because it inherits from obsolete velocity servlet (
 * which does not allow resource loading from webapp ). acc configuration is done like original velocity servlet does
 * 
...
public class WebWorkVelocityServlet extends VelocityViewServlet implements KeyConstants {
	...
	protected Context createContext(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response) {
		Context ctx = new NanocontainerVelocityContext((PicoContainer) (new RequestScopeObjectReference(request, REQUEST_CONTAINER)).get(),
	 ...
	 */
	protected Template handleRequest(javax.servlet.http.HttpServletRequest aRequest, javax.servlet.http.HttpServletResponse aResponse,
	...
	static class NanocontainerVelocityContext extends VelocityContext {
		...
		public Object internalGet(String key) {
			...
			if (stack.test(key.toString())) return stack.findValue(key);
