...
public class NanoWebServletTestCase extends MockObjectTestCase {
	...
	protected void setUp() throws Exception {
		...
		requestContainer.registerComponent(new ConverterComponentAdapter(Car.class, new ConstructorInjectionComponentAdapter(
				CarConverter.class, CarConverter.class)));
	...
	public void testParametersShouldBeSetAndActionExecuteInvoked() throws Exception {
		Mock actionFactory = mock(ActionFactory.class);
		nanoServletComponent = new NanoWebServletComponent((ActionFactory) actionFactory.proxy());

		// actionFactory
		MyAction action = new MyAction();
		actionFactory.expects(once()).method("getInstance").with(isA(PicoContainer.class), eq("/test")).will(returnValue(action));
		...
		// assertions
		assertEquals("execute", action.invoked);
	...
	public void testActionNonExecuteMethodsInvoked() throws Exception {
		Mock actionFactory = mock(ActionFactory.class);
		nanoServletComponent = new NanoWebServletComponent((ActionFactory) actionFactory.proxy());

		// url params
		requestMock.expects(atLeastOnce()).method("getParameterMap").will(returnValue(new HashMap()));
		...
		// path, action and view
		requestMock.expects(atLeastOnce()).method("getServletPath").will(returnValue("/test!anotherMethod.nwa"));
		requestMock.expects(atLeastOnce()).method("setAttribute").with(eq("year"), eq(new Integer(0)));
		requestMock.expects(once()).method("setAttribute").with(eq("cars"), eq(new Car[] {}));
		requestMock.expects(atLeastOnce()).method("setAttribute").with(eq("country"), eq(null));
		...
		// assertions
		assertEquals("anotherMethod", action.invoked);
	}
