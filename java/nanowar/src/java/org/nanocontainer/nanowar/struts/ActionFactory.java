...
/**
 * Uses Pico to produce Actions and inject dependencies into them.  This class does the bulk of the
 * work in nano-struts.  If you have your own <code>RequestProcessor</code> implementation, you can use
 * an <code>ActionFactory</code> in your <code>RequestProcessor.processActionCreate</code> method to
 * Picofy your Actions.
 *
...
public class ActionFactory {
    ...
    /**
     * Gets the <code>Action</code> specified by the mapping type from
     * a Pico container.  The action will be instantiated if
     * necessary, and its dependencies will be injected.  The action
     * will be instantiated via a special Pico container that just
     * contains actions.  If this container already exists in the
     * request attribute, this method will use it.  If no such
     * container exists, this method will create a new Pico container
     * and place it in the request.  The parent container will either
     * be the request container, or if that container can not be found
     * the session container, or if that container can not be found,
     * the application container.  If no parent container can be
     * found, a <code>PicoInitializationException</code> will be
     * thrown.
     * <p/>
     * The action path specified in the mapping is used as the
     * component key for the action.
     *
     * @param request the HTTP servlet request.
     * @param mapping the Struts mapping object.  The type property tells us what Action class is required.
     * @param servlet the Struts <code>ActionServlet.  The action is configured with this servlet.
    ...
    private MutablePicoContainer getActionsContainer(HttpServletRequest request) {
        MutablePicoContainer actionsContainer = (MutablePicoContainer)
                request.getAttribute(KeyConstants.ACTION_CONTAINER);
    ...
    private MutablePicoContainer getNanoServletContainer(HttpServletRequest request) throws PicoInitializationException {
        ...
        if (container == null) {
            container = (MutablePicoContainer) request.getSession().getServletContext().getAttribute(KeyConstants.APPLICATION_CONTAINER);
        }
        if (container == null) {
