 ...
 * <p>
 * This factory has two operating modes. By default it ensures, that every thread uses its own component at any time. This mode ({@link #ENSURE_THREAD_LOCALITY})
 * makes internal usage of a {@link ThreadLocalComponentAdapter}. If the application architecture ensures, that the thread that
 * creates the component is always also the thread that is th only user, you can set the mode {@link #THREAD_ENSURES_LOCALITY}.
 * In this mode the factory uses a simple {@link CachingComponentAdapter}that uses a {@link ThreadLocalReference}to cache the
 * component.
 * </p>
 * <p>
...
public class ThreadLocalComponentAdapterFactory extends DecoratingComponentAdapterFactory {
    ...
    /**
     * Constructs a wrapping ThreadLocalComponentAdapterFactory, that ensures the usage of the ThreadLocal. The Proxy instances
     * are generated by the JDK.
     * 
