...
public class AspectedTest {
    ...
    public void testGetComponentInstance() {
        mockery.checking(new Expectations() {
            {
                one(componentAdapter).getComponentInstance(with(same(container)),
                        with(same(ComponentAdapter.NOTHING.class)));
                will(returnValue("addComponent"));
                one(componentAdapter).getComponentKey();
                will(returnValue("componentKey"));
                one(applicator).applyAspects(with(same("componentKey")), with(same("addComponent")),
                        with(same(container)));
        ...
        Object component = adapter.getComponentInstance(container, ComponentAdapter.NOTHING.class);
