...
public class ConstructorInjector<T> extends SingleMemberInjector<T> {
	 ...
	 * @param monitor
	 *            the component monitor used by this addAdapter
	 * @param lifecycleStrategy
	 *            the component lifecycle strategy used by this addAdapter
	 * @param useNames
	 ...
	 */
	public ConstructorInjector(final Object componentKey,
			final Class componentImplementation, Parameter[] parameters,
	...
	protected Constructor<T> getGreediestSatisfiableConstructor(
			PicoContainer container) throws PicoCompositionException {
		...
		for (final Constructor<T> sortedMatchingConstructor : sortedMatchingConstructors) {
			...
			for (int j = 0; j < currentParameters.length; j++) {
				...
				try {
					...
					e.setAmbiguousDependency(parameterTypes[j]);
					throw e;
				}
			...
			if (greediestConstructor != null
					&& parameterTypes.length != lastSatisfiableConstructorSize) {
		...
		if (!conflicts.isEmpty()) {
			throw new PicoCompositionException(conflicts.size()
					+ " satisfiable constructors is too many for '"
					+ getComponentImplementation()
					+ "'. Constructor List:"
					+ conflicts.toString().replace(
							getComponentImplementation().getName(), "<init>")
							.replace("public <i", "<i"));
		} else if (greediestConstructor == null
				&& !unsatisfiableDependencyTypes.isEmpty()) {
			throw new UnsatisfiableDependenciesException(this,
					unsatisfiedDependencyType, unsatisfiableDependencyTypes,
					container);
		} else if (greediestConstructor == null) {
			...
			throw new PicoCompositionException(
					"Either the specified parameters do not match any of the following constructors: "
		...
		if (instantiationGuard == null) {
			instantiationGuard = new ThreadLocalCyclicDependencyGuard<T>() {
				public T run() {
					...
					try {
						Object[] parameters = getMemberArguments(
								guardedContainer, constructor);
						constructor = componentMonitor.instantiating(container,
								ConstructorInjector.this, constructor);
						long startTime = System.currentTimeMillis();
						T inst = newInstance(constructor, parameters);
						componentMonitor.instantiated(container,
								ConstructorInjector.this, constructor, inst,
								parameters, System.currentTimeMillis()
										- startTime);
						return inst;
					} catch (InvocationTargetException e) {
						componentMonitor.instantiationFailed(container,
								ConstructorInjector.this, constructor, e);
						...
						return caughtInstantiationException(componentMonitor,
								constructor, e, container);
					} catch (IllegalAccessException e) {
						return caughtIllegalAccessException(componentMonitor,
								constructor, e, container);
	...
	protected Object[] getMemberArguments(PicoContainer container,
		...
		return super.getMemberArguments(container, ctor, ctor
				.getParameterTypes());
	}
	...
	private List<Constructor<T>> getSortedMatchingConstructors() {
		...
		if (parameters == null) {
			Collections.sort(matchingConstructors,
					new Comparator<Constructor>() {
						public int compare(Constructor arg0, Constructor arg1) {
							return arg1.getParameterTypes().length
									- arg0.getParameterTypes().length;
						}
					});
		}
	...
	private Constructor<T>[] getConstructors() {
		return AccessController
				.doPrivileged(new PrivilegedAction<Constructor<T>[]>() {
					@SuppressWarnings("unchecked")
					public Constructor<T>[] run() {
						return (Constructor<T>[]) getComponentImplementation()
