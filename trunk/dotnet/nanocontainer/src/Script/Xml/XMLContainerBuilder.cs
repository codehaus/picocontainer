...
namespace NanoContainer.Script.Xml
{
	public class XMLContainerBuilder : AbstractFrameworkContainerBuilder
	{
		...
		private void RegisterComponentsAndChildContainers(CodeMemberMethod composeMethod, CodeVariableReferenceExpression picoContainerVariableRefr, XmlElement element)
		{
			...
			for (int i = 0; i < children.Count; i++)
			{
				if (children.Item(i) is XmlElement)
				{
					...
					if (CONTAINER.Equals(name))
					{
						...
						RegisterComponentsAndChildContainers(composeMethod, picoParentContainerVariableRefr, childElement);
					}
		...
		private CodeCompileUnit GenerateDomTreeFromXML(TextReader scriptCode)
		{
			...
			//DomTree Body
			RegisterComponentsAndChildContainers(composeMethod, picoContainerVariableRefr, rootNode);
		...
		{
			CodeCompileUnit script = GenerateDomTreeFromXML(scriptCode);
