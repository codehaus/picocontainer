...
public class Type1Util {
    ...
    /**
     * If the argument can be cast to a {@link MutablePicoContainer}, call {@link #enableBasicType1Support(MutablePicoContainer)}
     * on that argument. Otherwise, do nothing.
     * 
     * @param container the argument to try and send to {@link #enableBasicType1Support(MutablePicoContainer)}.
     */
    public static void tryToEnableBasicType1Support(final PicoContainer container) {
        if (container instanceof MutablePicoContainer)
            enableBasicType1Support((MutablePicoContainer) container);
    }
     ...
     */
    public static MutablePicoContainer enableBasicType1Support(final MutablePicoContainer target) {
     ...
     */
    public static Object handleType1Lifecycle(final Object key, final Object instance, final PicoContainer container) throws Exception {
    ...
    public static void addServiceManager(final MutablePicoContainer target) {
        target.registerComponentInstance(ServiceManager.class, new PicoContainerBasedServiceManager(target));
    }
    ...
    public static void addContext(final MutablePicoContainer target) {
        target.registerComponentInstance(Context.class, new PicoContainerBasedContext(target));
    }
    ...
    public static void addLogger(final MutablePicoContainer target) {
        ...
        if (target.getComponentAdapter(org.apache.log4j.Logger.class) != null) {
            logger = new Log4JLogger(
                    (org.apache.log4j.Logger) target.getComponentInstance(org.apache.log4j.Logger.class)
            );
        } else if (target.getComponentAdapter(org.apache.log.Logger.class) != null) {
            logger = new LogKitLogger((org.apache.log.Logger) target.getComponentInstance(org.apache.log.Logger.class));
        } else if (target.getComponentAdapter(Log.class) != null) {
            logger = new CommonsLogger((Log) target.getComponentInstance(Log.class));
        } else {
            logger = new NullLogger();
        }
        target.registerComponentInstance(Logger.class, logger);
    }
    ...
    public static Logger getLogger(final PicoContainer container) {
        return (Logger) container.getComponentInstance(Logger.class);
    }
    ...
    public static Context getContext(final PicoContainer container) {
        return (Context) container.getComponentInstance(Context.class);
    }
    ...
    public static ServiceManager getServiceManager(final PicoContainer container) {
        return (ServiceManager) container.getComponentInstance(ServiceManager.class);
    }
    ...
    public static Configuration getConfiguration(final Object key, final PicoContainer container) {
        ...
        final Object containerProvidedConfig = container.getComponentInstance(usedKey + CONFIGURATION_POSTFIX);
     ...
     * org.picocontainer.alternatives.ImplementationHidingComponentAdapter}.</p>
     * 
    ...
    /**
     * An {@link InvocationHandler} that is used from the {@link Type1Util#getActivityLifecycleProxy(Object)} method as
